...
public class Point implements Comparable<Point> {
    ...
    public int compareTo(Point that) {
        if((this.x == that.x)&&(this.y == that.y))
            return 0;
        if(this.y == that.y)
            return (this.x-that.x)/ Math.abs(this.x - that.x);
        else
            return (this.y -that.y)/ Math.abs(this.y-that.y);
    }
    ...
    private final class SlopeO implements Comparator<Point> {
                @Override
    ...
    public static void main(String[] args) {
        ...
        /*for(int i=0;i<5;i++){
            points[i] = new Point(StdRandom.uniform(10),StdRandom.uniform(10));
        }*/
